...
def check_ban(func):
    ...
    async def wrapper(client, message, *args, **kwargs):
        ...
        try:      
            is_banned = await Ani_otaku.is_user_banned(user_id)
            
            if is_banned:
                logger.debug(f"User {user_id} is banned")
                keyboard = InlineKeyboardMarkup(
                    [[InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ ʜᴇʀᴇ...!!", url="https://t.me/BotifyX_Support")]]
                )
            ...
            await message.reply_text(f"An unexpected error occurred: {e}. Please contact the developer @ITSANIMEN.")
            return
    ...
    async def wrapper(client, message, *args, **kwargs):
        ...
        async def is_sub(client, user_id, channel_id):
            try:
                ...
                mode = await Ani_otaku.get_channel_mode(channel_id) or await Ani_otaku.get_channel_mode_all(channel_id)
                if mode == "on":
                    exists = await Ani_otaku.req_user_exist(channel_id, user_id)
                    return exists
                return False
            except Exception as e:
        ...
        async def is_subscribed(client, user_id):
            channel_ids = await Ani_otaku.show_channels()
            if not channel_ids:
                return True
            if user_id == OWNER_ID:
                return True
        ...
        for chat_id in all_channels:
            ...
            if not is_member:
                try:
                    ...
                    name = data.title
                    mode = await Ani_otaku.get_channel_mode(chat_id)
...
async def start_command(client: Client, message: Message):
    ...
    await Ani_otaku.add_user(user_id, message)
